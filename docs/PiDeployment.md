...
WiFi Connect is the selected portal component. It uses NetworkManager to create
a temporary access point, collect a selected SSID and passphrase, save the
connection, and retry if joining fails. PiFrame owns when that component is
started and stopped; it does not delegate the appliance lifecycle to it.
...
| `piframe-provision.service` | Decides whether to invoke or stop WiFi Connect and supplies status to the screen. |
...
3. Package and pin WiFi Connect, then implement initial setup mode.

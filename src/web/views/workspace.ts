...
<html lang="en">
  ...
  <body><main>
    ...
    <script>
      const buttons = document.querySelectorAll("[data-section]"); const panels = document.querySelectorAll("[data-panel]"); const title = document.querySelector("#section-title"); const allFolders = document.querySelector("#all-folders"); const folderChoices = document.querySelector("#folder-choices"); const about = document.querySelector("#about-dialog"); const helpToc = document.querySelector("#help-toc"); const helpContent = document.querySelector("#help-content"); const labels = { dashboard: "Dashboard", general: "General", presentation: "Presentation", schedule: "Schedule", folders: "Albums", status: "System Status", help: "Help" }; const layoutSelect = document.querySelector("select[name=screenLayout]"); if (layoutSelect) { const adaptiveOption = layoutSelect.querySelector("option[value=multiple]"); if (adaptiveOption) { adaptiveOption.value = "triple"; adaptiveOption.textContent = "Three photos"; adaptiveOption.closest("label")?.querySelector("small")?.remove(); } const layoutTerm = Array.from(document.querySelectorAll("dt")).find((term) => term.textContent === "Layout"); if (layoutTerm?.nextElementSibling) layoutTerm.nextElementSibling.textContent = layoutSelect.value === "single" ? "One photo" : "Three photos"; }
      if (layoutSelect) { const restoredOption = layoutSelect.querySelector("option[value=triple]"); if (restoredOption) { restoredOption.value = "multiple"; restoredOption.textContent = "Adaptive"; const field = restoredOption.closest("label"); if (field && !field.querySelector("small")) field.insertAdjacentHTML("beforeend", "<small>PiFrame chooses a balanced one-, two-, or three-photo arrangement from the next photos.</small>"); } const layoutTerm = Array.from(document.querySelectorAll("dt")).find((term) => term.textContent === "Layout"); if (layoutTerm?.nextElementSibling) layoutTerm.nextElementSibling.textContent = layoutSelect.value === "single" ? "One photo" : "Adaptive"; }
    ...
    <script>
      (() => {
        ...
        const seconds = form?.querySelector('input[name="clockShowSeconds"]');
        const section = enabled?.closest(".section");
        if (!enabled || !format || !size || !date || !seconds || !section) return;

        const originalLabels = [enabled, format, size, date, seconds].map((control) => control.closest("label")).filter(Boolean);
        ...
        const secondsLabel = document.createElement("label"); secondsLabel.append(seconds, document.createTextNode(" Seconds"));
        toggles.append(dateLabel, secondsLabel);

...
<html lang="en">
  ...
  <body><main>
    <aside><div class="brand">${renderLogo(156)}</div><nav>${validSections.map((section) => `<button type="button" data-section="${section}"${section === activeSection ? " class=\"active\"" : ""}>${sectionLabel(section)}</button>`).join("")}</nav>${helpContents}<div id="open-frame-link" class="rail-footer"${activeSection === "general" ? " hidden" : ""}><a href="/display">Open frame</a></div></aside>
    ...
    <script>
      ...
      function selectSection(section) { buttons.forEach((button) => button.classList.toggle("active", button.dataset.section === section)); panels.forEach((panel) => panel.hidden = panel.dataset.panel !== section); if (helpToc) helpToc.hidden = section !== "help"; if (openFrameLink) openFrameLink.hidden = section === "general"; title.textContent = labels[section] || section; history.replaceState(null, "", "/?view=" + section); }

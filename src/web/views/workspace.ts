...
export function renderSettingsPage(context: AppContext, flash: FlashMessage, requestedSection: string | null): string {
  ...
  const systemActionControls = context.config.platform === "raspberry-pi"
    ...
    : `<section class="card system-actions" style="margin-top:16px"><h3>Power</h3><p class="muted">Restart and shutdown controls are available when PiFrame runs on a Raspberry Pi.</p></section>`;
...
<html lang="en">
  ...
  <body><main>
    ...
    <section class="content"><header class="top"><div><p class="muted">PiFrame Administration</p><h2 id="section-title">${sectionLabel(activeSection)}</h2></div></header>${renderFlash(flash)}
      ...
      <section class="panel" data-panel="status"${activeSection === "status" ? "" : " hidden"}><div class="dashboard-grid"><section class="card"><h3>System</h3><dl><div><dt>Platform</dt><dd><code>${escapeHtml(context.config.platform)}</code></dd></div><div><dt>Host</dt><dd><code>${escapeHtml(context.config.host)}:${context.config.port.toString()}</code></dd></div><div><dt>Data root</dt><dd><code>${escapeHtml(context.config.paths.dataRoot)}</code></dd></div><div><dt>Database</dt><dd><code>${escapeHtml(context.config.paths.databaseFile)}</code></dd></div></dl></section><section class="card"><h3>Storage</h3><dl><div><dt>Originals</dt><dd><code>${escapeHtml(context.config.paths.originalsDir)}</code></dd></div><div><dt>Thumbnails</dt><dd><code>${escapeHtml(context.config.paths.thumbnailsDir)}</code></dd></div><div><dt>Display assets</dt><dd><code>${escapeHtml(context.config.paths.displayDir)}</code></dd></div></dl></section></div>${systemActionControls}<section class="card" style="margin-top:16px"><h3>Recent activity</h3>${eventRows}</section></section>
    </section>

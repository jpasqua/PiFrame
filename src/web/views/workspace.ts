...
function renderGeneralSettingsPanel(settings: FrameSettings): string {
  ...
  return `<div class="card"><form method="post" action="/admin/general/save">
    ...
    <section class="section"><h3>Location and time</h3><label class="field">Location<input type="text" name="location" value="${escapeHtml(settings.location)}" maxlength="80" placeholder="City, state, country or postal code"><small>Used for future local information such as weather.</small></label><details><summary>Advanced location settings</summary><div class="advanced-location-fields"><label class="field">Time zone<input type="text" name="timeZone" value="${escapeHtml(settings.timeZone)}" list="time-zone-options" required><small>Used for scheduling and the clock.</small></label><datalist id="time-zone-options">${timeZones}</datalist><label class="field">Language<select name="language"><option value="en-US"${settings.language === "en-US" ? " selected" : ""}>English (United States)</option></select><small>More interface languages will be available when PiFrame is translated.</small></label></div></details></section>

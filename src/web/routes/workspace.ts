...
import { renderDisplaySettingsPage, renderFolderPhotosPage, renderFoldersPage, renderSettingsPage } from "../views/workspace.js";
...
export function handleWorkspaceGetRoute(context: AppContext, req: IncomingMessage, res: ServerResponse, url: URL): boolean {
  ...
  if (url.pathname === "/admin/folders") { sendHtml(res, 200, renderFoldersPage(context, readFlash(url))); return true; }
  if (url.pathname === "/admin/status") { redirect(res, "/?view=status"); return true; }
  if (url.pathname === "/admin/display") { sendHtml(res, 200, renderDisplaySettingsPage(context, readFlash(url))); return true; }
  return false;
}
